...
class MainViewController: UIViewController {
    ...
    private lazy var measureButton: UIBarButtonItem = {
        let button = UIBarButtonItem(title: "측정", style: .plain, target: self, action: #selector(pushMeasureViewController(_:)))
        return button
    }()
    ...
    @objc private func pushMeasureViewController(_ sender: Any) {
...
extension MainViewController: UITableViewDataSource {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.gyroList.count
    }
